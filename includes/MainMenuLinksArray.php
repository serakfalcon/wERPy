...
/* only instantiate if we don't have a valid cachefile. Not yet implemented
if (!file_exists($cachefile) || time() - $cachetime > filemtime($cachefile) || isset($_GET['clearCache'])) { */                    
// Should menu items be arranged this way?
    ...
    foreach ($MenuItems as $key => $item) {
        ...
        //add titles and the image link to each of the MenuItems arrays
        if ($key == 'system') {
            $MenuItems[$key]['Transactions']['title'] = _('General Setup Options');
            $MenuItems[$key]['Transactions']['src'] = $RootPath . '/css/' . $Theme . '/images/company.png';
            $MenuItems[$key]['Reports']['title'] =  _('Receivables/Payables Setup');
            $MenuItems[$key]['Reports']['src'] = $RootPath . '/css/' . $Theme . '/images/ar.png';
            $MenuItems[$key]['Maintenance']['title'] = _('Inventory Setup');
            $MenuItems[$key]['Maintenance']['src'] = $RootPath . '/css/' . $Theme . '/images/inventory.png';
        } else {
            $MenuItems[$key]['Transactions']['title'] = _('Transactions');
            $MenuItems[$key]['Transactions']['src'] = $RootPath . '/css/' . $Theme . '/images/transactions.png';
            $MenuItems[$key]['Reports']['title'] = _('Inquiries and Reports');
            $MenuItems[$key]['Reports']['src'] = $RootPath . '/css/' . $Theme . '/images/reports.png';
            $MenuItems[$key]['Maintenance']['title'] = _('Maintenance');
            $MenuItems[$key]['Maintenance']['src'] = $RootPath . '/css/' . $Theme . '/images/maintenance.png';
        }
    }
    foreach ($MenuItems as $key => $item) {
        unset($MenuItems[$key]['Transactions']['Caption']);
        unset($MenuItems[$key]['Transactions']['URL']);
        unset($MenuItems[$key]['Maintenance']['Caption']);
        unset($MenuItems[$key]['Maintenance']['URL']);
